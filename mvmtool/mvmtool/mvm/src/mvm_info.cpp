...
ConstUTF8 FieldInfo::GetName() {
    ConstPoolType ret = ClassLoader(class_ptr).GetConstPool(ReadUInt16(&data_offset[2]));
    return *(ConstUTF8 *)&ret;
}

ConstUTF8 FieldInfo::GetDescriptor() {
    ConstPoolType ret = ClassLoader(class_ptr).GetConstPool(ReadUInt16(&data_offset[4]));
    return *(ConstUTF8 *)&ret;
}
